...
function modalContent(event){
	...
	var sender = event.path[5].firstElementChild.firstElementChild.innerText;
...
function deleteContactMessage(event){
	...
	ajax("GET", "/admin/contact/messages/"+messageId+"/delete", null, messageDeleted, [event.path[5]]);
}

function messageDeleted(params, success, rensponseObj){
	...
	http.onreadystatechange = function(){
		if(http.readyState == XMLHttpRequest.DONE){
			if(http.status == 200){
				...
				alert('Category could not be saved, Please Try Again Later!');
